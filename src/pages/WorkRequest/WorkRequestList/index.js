...
const WorkRequestList = (props) => {
  ...
  const columns = useMemo(
    () => [
      ...
      //   name: "City",
      //   selector: (row) => (
      //     <TableText>{row?.address_province || "--"}</TableText>
      //   ),
      ...
      {
        name: "State",
        selector: (row) => <TableText>{row?.address_state || "--"}</TableText>,
      ...
      <div className="page-content">
        <Container fluid={true}>
          {props.dashboard ? null : (
            <BreadCrumb title={"Pending Work Request"} pageTitle="Work Order" />
          )}
          <Card>
            <CardBody>
...
export default withRouter(withTranslation()(WorkRequestList));
