...
import { getClientUserDetails } from "src/helpers/ClientMethod/client";
...
export default function StoreUserDetails() {
  ...
  const fetchUserDetails = async () => {
    ...
    try {
      const response = await getClientUserDetails({ id });
      setData(response[0] || {});
      setLoading(false);
    } catch (error) {
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <BreadCrumb title="User Details" pageTitle="Brand" />
        {loading ? (
          <Loading />
        ) : error ? (
          <Error500 />
